...
const addresses: Record<number, Address> = {
  ...
  43112: '0x988ef24e3e51308da71f44176f094ebaf18de78a',
...
function registryAddress (chainId: number) {
  ...
  throw new Error(`Unable to locate the Proposal you requested: ${chainId}`)
}
...
async function resolveAragon (domain: string, chainId: number, registryAddress: Address) {
  return new Promise<string>(async (resolve, reject) => {
    try {
      ...
      reject(new Error(`Unable to resolve the ID ${domain} on chain: ${chainId}`))
    }
  })
}
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const subscription = wrap.apps.subscribe(apps => {
        ...
        if (!appsSummary.kernel) return reject(new Error('Unable to locate HAL kernel'))
        if (!appsSummary.agent) return reject(new Error('Unable to connect with HAL'))
...
class Aragon {
  ...
  setup () {
    if (isConnected(this.chain) && !this.wrap && !this.inSetup) {
      setTimeout(() => {
        log.info('\n ** Setting Up New Proposal:', this.dao)
        this.inSetup = true
        ...
        try {
          options = {
            ...
            apm: { ipfs: { gateway: 'https://ipfs.io/ipfs' }, ensRegistryAddress: registryAddress(this.chain.id) }
          }
        } catch (e) {
          console.log('TODO: If HAL fails to deploy the proposal contract', e)
          return 
        }
  ...
  pathTransaction (tx: RPC.SendTransaction.TxParams, cb: Callback<RPC.SendTransaction.TxParams>) {
    if (!this.wrap) {
      this.setup()
      return cb(new Error('HAL is currently unreachable, please try again'))
    }
    ...
    this.wrap.calculateTransactionPath(this.actor, this.agent, 'execute', [tx.to, tx.value, tx.data]).then(result => {
      ...
      if (!newTx) return cb(new Error('Could not calculate a transaction path for HAL, make sure your acting account is a registered council member address'))
      delete newTx.nonce
