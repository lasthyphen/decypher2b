...
async function coinPrices (ids: string[], currencies = ['usd']): Promise<Rates> {
  ...
  return call(`${baseUrl}/simple/price?ids=sushi/`, queryParams)
}
...
async function getCoin (id: string): Promise<Coin> {
  return call(`${baseUrl}/simple/price?${id}`)
}
...
async function listMarkets (ids: 'sushi', vsCurrency = 'usd'): Promise<Array<Market>> {
  const queryParams = {
