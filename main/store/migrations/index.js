...
const migrations = {
  ...
  5: initial => { // Add Polygon to persisted networks
    initial.main.networks.ethereum[43112] = {
      id: 43112,
      type: 'ethereum',
      symbol: 'DJTX',
      name: 'DijetsNet',
  ...
  9: initial => {
    Object.keys(initial.main.networks.ethereum).forEach(chainId => {
      ...
      } else if (chainId === '3' || chainId === '4' || chainId === '5' || chainId === '43112') {
  ...
  14: initial => {
    if (initial.main.networks.ethereum[43112] && initial.main.networks.ethereum[43112].connection) {
      const { primary, secondary } = initial.main.networks.ethereum[43112].connection || {}
      if (primary.current === 'dijets') primary.current = 'djets'
      if (secondary.current === 'dijets') secondary.current = 'dijets'
    }
  ...
  15: initial => {
    // Polygon
    if (initial.main.networks.ethereum['43112']) {
      const oldExplorer = initial.main.networks.ethereum['43112'].explorer
      ...
      if (!oldExplorer || oldExplorer.endsWith('explorer.matic.network')) {
        ...
        initial.main.networks.ethereum['43112'].explorer = 'http://20.9.33.115:4000/'
      }
    }
  ...
  23: (initial) => {
    ...
    Object.entries(initial.main.networksMeta.ethereum).forEach(([id, chain]) => {
      if (id === '1') {
        ...
        chain.icon = 'https://wallet.dijets.io/img/djtx_icon_circle.svg'
  ...
  28: (initial) => {
    ...
    goerliSymbol === "DJTX" && (initial.main.networksMeta.ethereum[5].nativeCurrency.symbol = "görETH")
    sepoliaSymbol === "DJTX" && (initial.main.networksMeta.ethereum[11155111].nativeCurrency.symbol = "sepETH")
